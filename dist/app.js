...
function handleSaveItem(formData, editingId) {
    try {
        if (editingId) {
            ...
            const updatedItem = Object.assign(Object.assign({}, originalItem), { name: formData.name, category: formData.category, note: formData.note, link: formData.link, design: formData.design, targetDate: new Date(formData.targetDateTime).toISOString(), isRecurring: formData.isRecurring, 
                // Reset recurrence fields if not recurring anymore
                recurrenceInterval: formData.isRecurring ? formData.recurrenceInterval : undefined, recurrenceEndDate: formData.isRecurring && formData.recurrenceEndDate ? new Date(formData.recurrenceEndDate).toISOString() : null, 
...
function handleOrderChange(itemId, oldVisualIndex, newVisualIndex) {
    ...
    const currentListElements = listContainer === null || listContainer === void 0 ? void 0 : listContainer.querySelectorAll('.countdown-item');
    if (!currentListElements || currentListElements.length !== (listContainer === null || listContainer === void 0 ? void 0 : listContainer.children.length)) {
...
function checkPastEvents(forceRender = false) {
    ...
    countdownItems.forEach(item => {
        ...
        if (item.isPast || !item.targetDate)
            return;
...
function getNextValidOccurrence(item) {
    ...
    if (!item.isRecurring || !item.recurrenceInterval)
        return null;
    if (!currentDate)
        return null; // Need a starting date
