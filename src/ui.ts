import { CountdownItem, TimerDesign } from './models.js';
import { formatTimeRemaining, formatDisplayDate } from './utils.js';

let categoryFilter: string = 'all'; // State for current filter
let categories: Set<string> = new Set(); // Dynamically track categories

// --- DOM Element Selectors ---
const form = document.getElementById('add-item-form') as HTMLFormElement;
const formElements = {
    name: form.elements.namedItem('item-name') as HTMLInputElement,
    date: form.elements.namedItem('item-date') as HTMLInputElement,
    time: form.elements.namedItem('item-time') as HTMLInputElement,
    isRecurring: form.elements.namedItem('item-recurring') as HTMLInputElement,
    recurrenceInterval: form.elements.namedItem('item-recurrence-interval') as HTMLSelectElement,
    recurrenceEndDate: form.elements.namedItem('item-recurrence-end-date') as HTMLInputElement,
    category: form.elements.namedItem('item-category') as HTMLInputElement,
    note: form.elements.namedItem('item-note') as HTMLTextAreaElement,
    link: form.elements.namedItem('item-link') as HTMLInputElement,
    design: form.elements.namedItem('item-design') as HTMLSelectElement,
...
export function setupUIEventListeners(
    addItemCallback: (itemData: Omit<CountdownItem, 'id' | 'createdAt' | 'targetDate' | 'originalTargetDate' | 'isPast'> & { targetDateTime: string }) => void,
    dismissItemCallback: (id: string) => void,
    updateFilterCallback: (filter: string) => void,
    exportCallback: () => void,
    importCallback: (file: File) => void
): void {
    // Form Submission
    form.addEventListener('submit', (event) => {
        ...
        const newItemData = {
            ...
            recurrenceInterval: formElements.isRecurring.checked ? formElements.recurrenceInterval.value as 'daily' | 'weekly' | 'monthly' | 'yearly' : undefined,
    ...
    listContainer.addEventListener('click', (event) => {
        ...
        if (target.classList.contains('dismiss-button')) {
            const itemId = target.closest('.countdown-item')?.getAttribute('data-id');
            if (itemId) {
...
export function renderCountdownList(items: CountdownItem[]): void {
    ...
    filteredItems.sort((a, b) => {
        ...
        if (aIsPassed && !bIsPassed) return 1; // a is past, b is not -> b comes first
        if (!aIsPassed && bIsPassed) return -1; // b is past, a is not -> a comes first
...
function ensureHttp(link: string): string {
    if (!link) return '#'; // Return '#' or empty string if link is empty
